...
uint16_t DMA_ADC2_RX_BUF[DMA_ADC2_RX_BUF_SIZE] __attribute__((section(".ARM.__at_0x24000300")));	//! STM32H743VIT6 下必须改变默认数组存储位置，不然无法使用DMA1
...
void Samp_Adc2_Init(uint32_t buff_Addr, uint32_t trans_Num){
	...
	//? DMA配置
		...
		LL_DMA_InitTypeDef LL_DMA_Struct                        = {0};
		                   LL_DMA_Struct.PeriphRequest          = LL_DMAMUX1_REQ_ADC2;                // DMA请求外设：ADC2
		                   ...
		                   LL_DMA_Struct.PeriphOrM2MSrcAddress  = (uint32_t)&ADC2->DR;                // DMA发送数据源：ADC2数据寄存器
	...
	LL_ADC_REG_InitTypeDef ADC_REG_InitStruct                  = {0};
	                       ...
	                       ADC_REG_InitStruct.SequencerLength  = ADC2_SequencerLength_Num;            //! ADC采样序列长度：扫描
	...
	LL_ADC_REG_SetTriggerEdge(ADC2, LL_ADC_REG_TRIG_EXT_RISING);  // 设置触发源模式：上升沿触发

	//? 配置规则通道 
	LL_ADC_REG_SetSequencerRanks  (ADC2, LL_ADC_REG_RANK_1, ADC2_Sequence_Channel_1);          // 设置采样序列1 ADC通道
	LL_ADC_SetChannelSamplingTime(ADC2, ADC2_Sequence_Channel_1, ADC2_Sequence_Sample_Period);  // 设置采样周期
	LL_ADC_SetChannelSingleDiff   (ADC2, ADC2_Sequence_Channel_1, LL_ADC_SINGLE_ENDED);        // 设置单端口ADC或双端口ADC输入：单端输入

	LL_ADC_REG_SetSequencerRanks  (ADC2, LL_ADC_REG_RANK_2, ADC2_Sequence_Channel_2);          // 设置采样序列2 ADC通道
	LL_ADC_SetChannelSamplingTime(ADC2, ADC2_Sequence_Channel_2, ADC2_Sequence_Sample_Period);  // 设置采样周期
	LL_ADC_SetChannelSingleDiff   (ADC2, ADC2_Sequence_Channel_2, LL_ADC_SINGLE_ENDED);        // 设置单端口ADC或双端口ADC输入：单端输入

	LL_ADC_REG_SetSequencerRanks  (ADC2, LL_ADC_REG_RANK_3, ADC2_Sequence_Channel_3);          // 设置采样序列3 ADC通道
	LL_ADC_SetChannelSamplingTime(ADC2, ADC2_Sequence_Channel_3, ADC2_Sequence_Sample_Period);  // 设置采样周期
	LL_ADC_SetChannelSingleDiff   (ADC2, ADC2_Sequence_Channel_3, LL_ADC_SINGLE_ENDED);        // 设置单端口ADC或双端口ADC输入：单端输入

	LL_ADC_REG_SetSequencerRanks  (ADC2, LL_ADC_REG_RANK_4, ADC2_Sequence_Channel_4);          // 设置采样序列4 ADC通道
	LL_ADC_SetChannelSamplingTime(ADC2, ADC2_Sequence_Channel_4, ADC2_Sequence_Sample_Period);  // 设置采样周期
	LL_ADC_SetChannelSingleDiff   (ADC2, ADC2_Sequence_Channel_4, LL_ADC_DIFFERENTIAL_ENDED);  // 设置单端口ADC或双端口ADC输入：差分输入

	LL_ADC_REG_SetSequencerRanks  (ADC2, LL_ADC_REG_RANK_5, ADC2_Sequence_Channel_5);          // 设置采样序列5 ADC通道
	LL_ADC_SetChannelSamplingTime(ADC2, ADC2_Sequence_Channel_5, ADC2_Sequence_Sample_Period);  // 设置采样周期
	LL_ADC_SetChannelSingleDiff   (ADC2, ADC2_Sequence_Channel_5, LL_ADC_DIFFERENTIAL_ENDED);  // 设置单端口ADC或双端口ADC输入：差分输入
	...
	//? 设置ADC传输方式
	#if __HARDWARE_CONFIG__SAMPLE_ADC2_DMA_ENABLE__ // begin of __HARDWARE_CONFIG__SAMPLE_ADC2_DMA_ENABLE__
		LL_ADC_REG_SetDataTransferMode(ADC2, LL_ADC_REG_DMA_TRANSFER_UNLIMITED);						  // DMA无限制传输
	#else
		LL_ADC_REG_SetDataTransferMode(ADC2, LL_ADC_REG_DR_TRANSFER);                                     // 数据寄存器传输
		LL_ADC_EnableIT_EOC           (ADC2);                                                             // 使能E0C中断
...
void Start_Sample_adc2(){
	#if __HARDWARE_CONFIG__SAMPLE_ADC2_DMA_ENABLE__ // begin of __HARDWARE_CONFIG__SAMPLE_ADC2_DMA_ENABLE__	
		LL_DMA_EnableStream(DMA1, LL_DMA_STREAM_3);						 // 使能DMA
	#endif // end of __HARDWARE_CONFIG__SAMPLE_ADC2_DMA_ENABLE__

	LL_ADC_Enable             (ADC2);                                    // 使能ADC
	while                     (LL_ADC_IsActiveFlag_ADRDY(ADC2) != SET);  // 等待ADC准备就绪
	LL_ADC_REG_StartConversion(ADC2);                                    // 开启ADC转换
...
void Stop_Sample_adc2(){
	...
	LL_ADC_REG_StopConversion (ADC2);                                       // 停止ADC转换
	while(LL_ADC_REG_IsConversionOngoing(ADC2) != 0);  						// 等待转换停止
	LL_ADC_Disable            (ADC2);                                       // 关闭ADC
	LL_ADC_EnableDeepPowerDown(ADC2);                                       // 令ADC处于低功耗状态

	#if __HARDWARE_CONFIG__SAMPLE_ADC2_DMA_ENABLE__ // begin of __HARDWARE_CONFIG__SAMPLE_ADC2_DMA_ENABLE__
		LL_DMA_DisableStream(DMA1, LL_DMA_STREAM_3);  // 关闭DMA通道
		LL_DMA_ClearFlag_TC3(DMA1);                   // 清除TC标志位
	#endif // end of __HARDWARE_CONFIG__SAMPLE_ADC2_DMA_ENABLE__
}
...
//-----------------------------------------------------------------
void DMA1_Stream3_IRQHandler_Func(){
	if(LL_DMA_IsActiveFlag_TC3(DMA1) == SET){ // 用于检测采样率是否准确的（1M采样率时，DMA传输1000个数据大概时间是1ms）
...
void ADC_IRQHandler(void)
{
	if(LL_ADC_IsActiveFlag_EOC(ADC2) != RESET){ // 不使用DMA时，在这里面获取采样值
