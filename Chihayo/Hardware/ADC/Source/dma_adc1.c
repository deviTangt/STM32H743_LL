...
uint16_t DMA_ADC1_RX_BUF[DMA_ADC1_RX_BUF_SIZE] __attribute__ ((section ("DMA_ADC1_RX_BUF"),  zero_init));	//! STM32H743VIT6 下必须改变默认数组存储位置，不然无法使用DMA1
...
void AD_TIM15_Init(){
	...
	LL_TIM_InitTypeDef LL_TIM_Struct               = {0};
	                   LL_TIM_Struct.Prescaler     = 24 - 1;                      //? 预分频系数
	                   LL_TIM_Struct.Autoreload    = 100 - 1;                     //? 周期系数
...
void ADC_IRQHandler(void)
{
	if(LL_ADC_IsActiveFlag_EOC(ADC1) != RESET){ // 不使用DMA时，在这里面获取采样值
